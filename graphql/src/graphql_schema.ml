...
module Make (Io : IO) (Field_error : Field_error) = struct
  ...
  let io_field_with_set_context ?doc ?(deprecated = NotDeprecated) name ~typ
      ~args ~resolve =
    Field
      {
        name;
        ...
        typ;
        args;
        resolve = `Resolve_with_set_child_context resolve;
        lift = id;
      }
          ...
          Field
            {
              ...
              typ;
              args;
              resolve = `Resolve (fun ctx () -> resolve ctx);
            })
        fields
    in
     ...
     fun ?(memo = ([], StringSet.empty)) typ ->
      ...
      | Object o as obj ->
          unless_visited memo o.name (fun (result, visited) ->
              ...
              in
              let memo' =
                List.fold_left reducer (result', visited')
                  (Lazy.force o.fields)
              in
              let abstracts_reducer memo a = types ~memo (Abstract a) in
                    ...
                    resolve =
                      `Resolve
                        (fun _ t ->
                          ...
                          | AnyTyp (Enum e) ->
                              Some
                                (List.map (fun x -> AnyEnumValue x) e.values)
