...
struct
  ...
  let json_err = function Ok _ as ok -> ok | Error err -> Error (`String err)
  ...
  let make_callback :
   ...
   fun make_context schema _conn (req : Cohttp.Request.t) body ->
    ...
    | `GET, [ "graphql" ] ->
        if
          Cohttp.Header.get req.Cohttp.Request.headers "Connection"
          = Some "Upgrade"
          && Cohttp.Header.get req.headers "Upgrade" = Some "websocket"
        then
          let handle_conn =
        ...
        else static_file_response "index.html"
